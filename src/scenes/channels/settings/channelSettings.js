    ...
    try {
        ...
        if (channel) {
            ...
            await ctx.editMessageText(
                `Настройки канала:\nPost price: ${channel.price} ₽\nCategory: ${channel.category}\nTime of active post: ${channel.timeOfActivePost.split(':')[0]} часов ${channel.timeOfActivePost.split(':')[1]} минут\nAutoposting: ${channel.autoposting}`,
                Extra.HTML().markup((m) =>
                    m.inlineKeyboard([
...
channelSettings.on("callback_query", async (ctx) => {
    try {
        ...
        if (data.action === "back") {
            ...
            await ctx.scene.enter("changeAutoposting").catch((e) => console.log(e.message));
