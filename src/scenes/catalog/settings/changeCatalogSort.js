...
changeCatalogSort.enter(async (ctx) => {
    try {
        ...
        let filter = await Filter.findOne({userId: ctx.update.callback_query.message.chat.id});
        if (filter) {
            ...
            await ctx.editMessageText(`Сортировка: ${sorting}`, Extra.markdown().markup((m) => m.inlineKeyboard(keyboard)));
